...
# Global calibration variables for yaw and pitch
calibration_data = {
    'yaw_neutral': 0.0,
    'yaw_min': None,
    'yaw_max': None,
...
# Global keyboard state for pynput to calibrate yaw and pitch
keyboard_state = {
    't': False,
    'o': False,
    'q': False
...
def keyboard_calibration_handler():
    global keyboard_state, keyboard_state_lock
    
    if not KEYBOARD_AVAILABLE:
        print("[WARNING] pynput library not available. Calibration disabled.")
        return
    
    print("[CALIBRATION] Press 't' to calibrate yaw (hold while moving hand)")
    print("[CALIBRATION] Press 'o' to calibrate pitch (hold while moving hand)")
    ...
    def on_press(key):
        ...
        try:
            with keyboard_state_lock:
                if hasattr(key, 'char') and key.char in ['t', 'o', 'q']:
                    keyboard_state[key.char] = True
                    print(f"[DEBUG] Key '{key.char}' pressed and added to set")
        ...
        try:
            with keyboard_state_lock:
                if hasattr(key, 'char') and key.char in ['t', 'o', 'q']:
                    ...
                    print(f"[DEBUG] Key '{key.char}' released and removed from set")
    ...
    while True:
        try:
            ...
            print(f"[CALIBRATION] Error in pynput handler: {e}")
            break
...
def is_key_pressed(key):
    ...
    try:
        with keyboard_state_lock:
            is_pressed = keyboard_state.get(key, False)
            if is_pressed:
...
class MediaPipeHandTracker:
    ...
    # Hand orientation matrices (From anyteleop : dex-retargeting)
...
def start_hand_tracking(queue: multiprocessing.Queue, depth_queue: multiprocessing.Queue, hand_type: str):
    ...
    try:
        keyboard_thread = threading.Thread(target=keyboard_calibration_handler, daemon=True)
        keyboard_thread.start()
    except Exception as e:
        print(f"[WARNING] Could not start keyboard handler: {e}")
        print("[WARNING] Calibration features will not be available")
    ...
    while True:
        ...
        if joint_pos is None:
            ...
            with joint_data_lock:
                ...
                latest_joint_data['hand_confidence'] = hand_confidence 
...
def produce_frame_from_network(queue, depth_queue, host='127.0.0.1', port=9999):
...
def map_index_joints(mcp_flex_deg, pip_flex_deg):
    """
    Map direct finger angle values from RGB retargeting to hand joint positions.
...
def map_yaw_pitch(yaw_raw_deg=0.0, pitch_raw_deg=0.0):
    """
    Map RGB retargeting yaw/pitch values to adapt hand joint positions for wrist orientation.
...
def get_joint_data():
    ...
    with joint_data_lock:
        ...
        if hand_confidence <= 0.0:
            # Return default values when no hand is detected
            return {
        ...
        mapped_wrist = map_yaw_pitch(
            latest_joint_data.get('yaw_raw_deg', 0.0),
            latest_joint_data.get('pitch_raw_deg', 0.0)
        )
...
def start_joint_data_client(target_host='localhost', target_port=8890):
...
def main(hand_type="Right", camera_host="127.0.0.1", camera_port=9999):
    """
    ...
    print("• Press and hold 'Y' to calibrate yaw (left/right wrist rotation)")
    print("  - When Y is first pressed, current position becomes neutral (maps to 0)")
    print("  - While holding Y, move hand to extremes - min value maps to -120, max to +120")
    print("• Press and hold 'P' to calibrate pitch (up/down wrist rotation)")
    print("  - When P is first pressed, current position becomes neutral (maps to 0)")  
    print("  - While holding P, move hand to extremes - min value maps to -120, max to +120")
