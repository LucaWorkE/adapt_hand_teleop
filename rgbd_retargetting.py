...
def pixel_to_world_coordinates(pixel_x, pixel_y, depth_mm):
    if not camera_intrinsics.intrinsics_valid or depth_mm <= 0:
    ...
    try:
        ...
        world_x = (pixel_x - camera_intrinsics.ppx) * depth_m / camera_intrinsics.fx
        world_y = (pixel_y - camera_intrinsics.ppy) * depth_m / camera_intrinsics.fy
        world_z = depth_m
...
def landmark_to_world_coordinates(keypoint_2d, depth_frame, landmark_idx):
    """
    ...
    try:
        ...
        pixel_x = landmark.x * depth_frame.shape[1]  # Convert normalized to pixel coords
        ...
        world_x, world_y, world_z = pixel_to_world_coordinates(pixel_x, pixel_y, depth_mm)
...
def calculate_single_finger_angles(joint_pos, finger_landmarks):
    ...
    Default calculation if depth is not available.
    """
    if joint_pos is None:
        return None
        
    try:
        wrist_pos = joint_pos[0]  # Wrist position
...
def calculate_finger_angles_for_finger_enhanced(joint_pos, finger_landmarks, keypoint_2d, depth_frame):
    """
    Same as calculate_single_finger_angles but uses depth data for z coordinate instead of inferred depth
    """
    if joint_pos is None:
        return None
    ...
    try:
        ...
        landmark_indices = [0] + finger_landmarks  # All finger landmarks including tip
        
        for i, (name, idx) in enumerate(zip(landmark_names, landmark_indices)):
            world_x, world_y, world_z = landmark_to_world_coordinates(keypoint_2d, depth_frame, idx)
        ...
        # Debug info about depth enhancement usage
        ...
        return {
            ...
            'pip_flex': pip_flex_angle,
            'dip_flex': dip_flex_angle,
            'depth_enhanced_count': depth_used_count  # Track depth enhancement usage
        }
...
def calculate_direct_finger_angles_depth_enhanced(joint_pos, keypoint_2d, depth_frame):
    """
    Calculate finger joint angles directly from MediaPipe landmarks for all fingers using enhanced depth data.
    
    Args:
        joint_pos: MediaPipe joint positions array (fallback)
        keypoint_2d: MediaPipe 2D landmarks for depth extraction
        depth_frame: Intel RealSense depth frame
        
    Returns:
        dict: Direct joint angles in radians for all fingers
    """
    if joint_pos is None:
        return None
    ...
    try:
        ...
        # Calculate angles for each finger using depth enhancement
        for finger_name, landmarks in finger_landmarks.items():
            finger_angles = calculate_finger_angles_for_finger_enhanced(
                joint_pos, landmarks, keypoint_2d, depth_frame
            )
            if finger_angles is not None:
                all_finger_angles[finger_name] = finger_angles
                # Track depth enhancement usage
                if 'depth_enhanced_count' in finger_angles:
                    total_depth_enhanced += finger_angles['depth_enhanced_count']
        ...
        if 'index' in all_finger_angles:
            ...
            result['total_depth_enhanced'] = total_depth_enhanced
            return result
        else:
            return None
...
def calculate_direct_finger_angles(joint_pos):
    """
    Calculate finger joint angles directly from MediaPipe landmarks for all 4 fingers.
...
def calculate_thumb_joint_angles_depth_enhanced(joint_pos, keypoint_2d, depth_frame):
    """
    Calculate thumb joint angles from MediaPipe landmarks using enhanced depth data.
    ...
    try:
        # Try to get enhanced 3D positions using actual depth data for thumb landmarks
        thumb_landmark_indices = [0, 1, 2, 3, 4]  # wrist, cmc, mcp, ip, tip
        enhanced_positions = {}
        depth_used_count = 0
        
        for i, idx in enumerate(thumb_landmark_indices):
            world_x, world_y, world_z = landmark_to_world_coordinates(keypoint_2d, depth_frame, idx)
            if world_x is not None:
                enhanced_positions[idx] = np.array([world_x, world_y, world_z])
                depth_used_count += 1
            else:
                # Fallback to MediaPipe inferred 3D position
                enhanced_positions[idx] = joint_pos[idx]
        
        # Use enhanced positions
        wrist_pos = enhanced_positions[0]    # Wrist position (reference)
        cmc_pos = enhanced_positions[1]      # CMC joint (base of thumb)
        mcp_pos = enhanced_positions[2]      # MCP joint 
        ip_pos = enhanced_positions[3]       # IP joint (equivalent to PIP)
        tip_pos = enhanced_positions[4]      # Thumb tip
...
def calculate_thumb_joint_angles(joint_pos):
    """
    Calculate thumb joint angles from MediaPipe landmarks. uses the inferred 3D positions.
...
def extract_independent_yaw_pitch(rotation_matrix):
    """
    Extract yaw and pitch from rotation matrix.
    
    Args:
        rotation_matrix: 3x3 rotation matrix from MediaPipe
...
def extract_roll_v2(rotation_matrix):
    """
...
class MediaPipeHandTracker:
    """
    MediaPipe-based hand tracking system that detects hand landmarks and converts them to MANO coordinate space.
    ...
    def __init__(self, hand_type="Right", min_detection_confidence=0.8, min_tracking_confidence=0.8, selfie=False):
        ...
        # Removed unused detected_hand_type variable
    ...
    def detect(self, rgb_image):
        ...
        # Get confidence from MediaPipe detection (same as RGB source)
...
def start_hand_tracking(queue: multiprocessing.Queue, depth_queue: multiprocessing.Queue, hand_type: str):
    ...
    while True:
        ...
        if joint_pos is None:
            ...
            # Extract orientation
            yaw_independent, pitch_independent = extract_independent_yaw_pitch(wrist_rot_matrix)
            roll_v2 = extract_roll_v2(wrist_rot_matrix)
            ...
            # Calculate 3D world coordinates from depth
                ...
                try:
                    ...
                    if wrist_depth_mm > 0:
                        wrist_world_x, wrist_world_y, wrist_world_z = pixel_to_world_coordinates(
                            wrist_camera_x, wrist_camera_y, wrist_depth_mm
                        )
                except Exception as e:
                    pass
            
            # Calculate finger angles using depth-enhanced methods
            ...
            # Use direct MediaPipe confidence (same as RGB source)
...
def main(hand_type="Right", camera_host="192.168.1.152", camera_port=9998):
    """
