...
def design_scene():
    ...
    ur5_cfg = ArticulationCfg(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            pos=(0.0, 0.0, 1.05),
            joint_pos={
                # Initialize arm joints to resting positions
                ...
                "wrist_2_joint": 0.0,
                "wrist_3_joint": 0.0,
                # Initialize hand joints to neutral positions
                "Wrist_Yaw": 0.0,
                "Wrist_Pitch": 0.0,
                ...
                "Index_PIP": -0.5,
                "Index_DIP": -1.0,         
                # Other fingers in neutral positions
                "Middle_MCP": -0.5,
                "Middle_PIP": -0.5,
                "Pinky_MCP": -0.5,
                "Pinky_PIP": -0.5,
        ...
        actuators={
            ...
            "thumb_joints": ImplicitActuatorCfg(
                joint_names_expr=["Thumb_.*"],  # All thumb joints (CMC1, CMC2, MCP, IP)
...
class DualSourceDataReceiver:
    ...
    def _update_combined_data(self):
        ...
        with self.combined_lock:
            ...
            # Update global data for backward compatibility
...
rgb_wrist_smoothing = 0.8      # High smoothing for wrist
rgb_roll_smoothing = 0.012     # Slightly higher smoothing for roll
rgb_finger_smoothing = 0.7     # Medium smoothing for fingers
rgb_thumb_smoothing = 0.85     # Higher smoothing for thumb
...
def apply_rgb_joint_smoothing_and_safety(new_joints, joint_type="finger"):
    ...
    Args:
        new_joints: Dict of new joint positions
        joint_type: Type of joints ("wrist", "finger", "thumb")
    
    Returns:
        Dict of smoothed and safety-limited joint positions
    """
    ...
    for joint_name, new_value in new_joints.items():
        ...
        # Apply maximum change per frame limit
        change = smoothed_value - prev_value
        if abs(change) > max_change:
            change = max_change if change > 0 else -max_change
            smoothed_value = prev_value + change
        
        # Apply joint-specific safety bounds
        if "MCP" in joint_name or "PIP" in joint_name:
            smoothed_value = np.clip(smoothed_value, -1.8, 0.2)  # Conservative bounds
...
def map_rgb_finger_angles_to_isaac_joints(mcp_spread_deg, mcp_flex_deg, pip_flex_deg, dip_flex_deg):
    """
    Map direct finger angle values from RGB retargeting to Isaac Lab joint positions.
...
def map_rgb_retargeting_to_isaac_joints(yaw_deg=140.0, pitch_deg=-70.0, roll_deg=0.0):
    ...
    wrist_joints = {
        "Wrist_Yaw": wrist_yaw_joint,            # Control yaw with Wrist_Yaw
        "Wrist_Pitch": wrist_pitch_joint,        # Control pitch with Wrist_Pitch
        "wrist_3_joint": wrist_roll_joint        # Control roll with wrist_3_joint (UR5 arm)
    }
    ...
    smoothed_wrist_joints["wrist_3_joint"] = wrist_roll_joint  # Add back the already-processed roll joint
...
def main():
    ...
    # Design scene and get robot
    robot = design_scene()
    
    # Play the simulator
    sim.reset()
    
    # Start the dual source data receiver
    ...
    # Check for arm roll joint (wrist_3_joint)
    ...
    fixed_positions = {
        ...
        "Index_MCP_Spread": -0.1,  # Fixed
        "Index_DIP": -1.0,         # Fixed
        # Other finger DIP joints fixed
        "Middle_DIP": -1.0,        # Fixed
        "Ring_DIP": -1.0,          # Fixed  
        "Pinky_DIP": -1.0,         # Fixed
    }
    ...
    while simulation_app.is_running():
        # Get retargeting data
        retargeting_data = receiver.get_retargeting_data()
        
        # Create joint position targets starting with current positions (SAFER)
        ...
        if retargeting_data:
            ...
            # Check if we have valid thumb data
            ...
            # Check if we have valid wrist orientation data
            ...
            if has_finger_data:
                # Map retargeting finger angles to joint positions
                finger_joints = {}
            ...
            if has_thumb_data:
                ...
                # Apply thumb joint controls
                for i, joint_name in enumerate(joint_names):
                    if joint_name in thumb_joints:
                        joint_pos_target[0, i] = thumb_joints[joint_name]
            
            if has_wrist_data:
                # Map retargeting wrist orientation to joint positions
                ...
                # Apply wrist joint controls
                for i, joint_name in enumerate(joint_names):
                    if joint_name in wrist_joints:
                        joint_pos_target[0, i] = wrist_joints[joint_name]
            
            # Print control status every 50 steps
            if count % 50 == 0:
                pass  # Removed detailed status prints for cleaner output
        else:
            # No data available - print waiting message
            if count % 50 == 0:
                pass  # Removed waiting message for cleaner output
        ...
        # Print control commands being sent to Isaac (every frame)
        if count % 30 == 0:  # Print every 30 frames to avoid spam
            active_commands = []
            for i, joint_name in enumerate(joint_names):
            ...
            if active_commands:
                # print(f"{', '.join(active_commands)}")  # Commented out joint commands
                pass
            else:
                # print("🎮 ACTIVE COMMANDS: None (holding current positions)")  # Commented out
                pass
...
if __name__ == "__main__":
    ...
    # Run the main Isaac Lab simulation
    main()
